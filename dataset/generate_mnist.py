...
def generate_mnist(dir_path, num_clients, num_classes, niid, balance, partition):
    ...
    """
    X, y, statistic = separate_data((trainset.data, trainset.targets), num_clients, num_classes,
                                    niid, balance, partition, class_per_client=2)
    train_data, test_data = [], []
    for data, target in zip(X, y):
        train_data.append({'x': data, 'y': target})
    test_data.append({'x': testset.data, 'y': testset.targets})
    """
